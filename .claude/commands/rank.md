...
Dispatch parallel `general-purpose` subagents (Claude Code: Agent tool; opencode: Task tool with `subagent_type: general`), ~5 jobs per agent (a single agent is fine for ≤5 jobs). Token-efficiency rules, consistent with `/apply`:
