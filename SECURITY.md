...
- **Permission allowlist**: `.claude/settings.json` (Claude Code) and `opencode.json` (opencode, mirroring it) pre-approve only the specific commands the workflow needs; the `security-guards` CI job fails any PR that widens `.claude/settings.json`, adds package-manifest lifecycle scripts, or weakens the personal-data gitignore rules. Note the allowlist governs Bash commands - the model's native WebFetch/WebSearch tools are outside its reach, which is exactly why the instruction-level rules above exist. If you change one permission file, keep the other in sync.
